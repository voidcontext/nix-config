{ pkgs, pkgsUnstable, modulesPath, home-manager, nix-config-extras, blog, blog-beta, config, ... }:
...
in
{
  sops.defaultSopsFile = ../../secrets/deneb.yaml;
  
  sops.secrets."vdx/authorized_keys" = {};
  ...
  users.users.vdx = {
    ...
    openssh.authorizedKeys.keyFiles = [
      config.sops.secrets."vdx/authorized_keys"
    ];
  };
